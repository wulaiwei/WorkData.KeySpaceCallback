...
namespace Dapper
{
    internal abstract class SqlDialectBase : ISqlDialect
    {
        ...
        public virtual string GetColumnName(string prefix, string columnName, string alias)
        {
            ...
            {
                throw new ArgumentNullException("columnName", "列名不能为 NULL 或 Empty");
            }
        ...
        public virtual string GetNextSequence(string sequenceName)
        {
            throw new NotSupportedException("此数据库不支持 Sequence");            
        }

        public virtual string GetCurrentSequence(string sequenceName)
        {
            throw new NotSupportedException("此数据库不支持 Sequence");
        }
