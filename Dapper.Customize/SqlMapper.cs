...
#else
        public CommandDefinition(string commandText, object parameters = null, IDbTransaction transaction = null,
                ...
                Type parametersType, Type[] otherTypes, int gridIndex)
            {
                ...
                unchecked
                {
                    ...
                    hashCode = hashCode*23 +
                               (connectionString == null ? 0 : connectionStringComparer.GetHashCode(connectionString));
...
#else
            this IDbConnection cnn, string sql, Func<TFirst, TSecond, TReturn> map, object param = null,
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, object param, IDbTransaction transaction, bool buffered, string splitOn, int? commandTimeout, CommandType? commandType
        ...
        public static IEnumerable<TReturn> Query<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(
            ...
            CommandType? commandType = null)
        {
            return MultiMap<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(cnn, sql, map, param,
                transaction, buffered, splitOn, commandTimeout, commandType);
        }
        ...
        /// <param name="commandType">Is it a stored proc or a batch?</param>
        /// <returns></returns>
        public static IEnumerable<TReturn> Query<TReturn>(this IDbConnection cnn, string sql, Type[] types,
            Func<object[], TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true,
            ...
            bool finalize)
        {
            ...
            var param = command.Parameters;
            identity = identity ??
                       new Identity(command.CommandText, command.CommandType, cnn, types[0],
                           param == null ? null : param.GetType(), types);
            var cinfo = GetCacheInfo(identity, param, command.AddToCache);
            ...
            try
            {
                ...
                var mapIt = GenerateMapper(types.Length, deserializer.Func, otherDeserializers,
                    map);
        ...
        private static Func<IDataReader, TReturn> GenerateMapper<TReturn>(int length,
            Func<IDataReader, object> deserializer, Func<IDataReader, object>[] otherDeserializers,
        ...
        private static CacheInfo GetCacheInfo(Identity identity, object exampleParameters, bool addToCache)
        {
            ...
            if (!TryGetQueryCache(identity, out info))
            {
                ...
                if (identity.parametersType != null)
                {
                    ...
                    if ((identity.commandType == null || identity.commandType == CommandType.Text) &&
            ...
            cmd.CommandText = pseudoPositional.Replace(cmd.CommandText, match =>
            {
                ...
                if (parameters.TryGetValue(key, out param))
                {
                    if (firstMatch)
                    {
                        ...
                        // only clear if we are pretty positive that we've found this pattern successfully
                    }
        ...
        private static IEnumerable<PropertyInfo> FilterParameters(IEnumerable<PropertyInfo> parameters, string sql)
        {
            return
                parameters.Where(
                    p =>
                        Regex.IsMatch(sql, @"[?@:]" + p.Name + "([^a-z0-9_]+|$)",
                            RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant));
        }

        // look for ? / @ / : *by itself*
        private static readonly Regex smellsLikeOleDb = new Regex(@"(?<![a-z0-9@_])[?@:](?![a-z0-9@_])",
            RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant | RegexOptions.Compiled),
            literalTokens = new Regex(@"(?<![a-z0-9_])\{=([a-z0-9_]+)\}",
                RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.CultureInvariant | RegexOptions.Compiled),
            pseudoPositional = new Regex(@"\?([a-z_][a-z0-9_]*)\?",
            ...
            bool removeUnused, IList<LiteralToken> literals)
        {
            ...
            foreach (var prop in props)
            {
                    ...
                    // https://connect.microsoft.com/VisualStudio/feedback/details/381934/sqlparameter-dbtype-dbtype-time-sets-the-parameter-to-sqldbtype-datetime-instead-of-sqldbtype-time
                {
                    ...
                    if (dbType == DbType.Object && prop.PropertyType == typeof (object)) // includes dynamic
                    {
                        ...
                        il.Emit(OpCodes.Call,
                            typeof (SqlMapper).GetMethod("GetDbType", BindingFlags.Static | BindingFlags.Public));
                ...
                if (checkForNull)
                {
                    ...
                    var allDone = (dbType == DbType.String || dbType == DbType.AnsiString)
                        ? il.DefineLabel()
                        : (Label?) null;
...
#pragma warning restore 618
        ...
        private static readonly Dictionary<TypeCode, MethodInfo> toStrings = new[]
        {
            ...
            typeof (decimal)
        }.ToDictionary(x => TypeExtensions.GetTypeCode(x),
            x => x.GetPublicInstanceMethod("ToString", new[] {typeof (IFormatProvider)}));
        ...
        private static readonly MethodInfo StringReplace = typeof (string).GetPublicInstanceMethod("Replace",
        ...
        private static IDataReader ExecuteReaderImpl(IDbConnection cnn, ref CommandDefinition command,
            CommandBehavior commandBehavior, out IDbCommand cmd)
        {
...
#pragma warning restore 618
        ...
        public partial class GridReader : IDisposable
        {
            ...
            public GridReader(IDbCommand command, IDataReader reader, Identity identity, IParameterCallbacks callbacks,
                bool addToCache)
            {
                    ...
                    foreach (
                        var r in
                            SqlMapper.MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TSixth, TSeventh, TReturn>(
                                null, default(CommandDefinition), func, splitOn, reader, identity, false))
                    {
                        yield return r;
                    }
                }
    ...
    /// </summary>
        ...
        // one per thread
        [ThreadStatic] private static StringBuilder perThreadStringBuilderCache;
        ...
        SqlMapper.IParameterCallbacks
    {
        ...
        private readonly Dictionary<string, Action<object, DynamicParameters>> cachedOutputSetters =
            new Dictionary<string, Action<object, DynamicParameters>>();
...
#pragma warning restore 612, 618
    ...
    sealed partial class TableValuedParameter : SqlMapper.ICustomQueryParameter
    {
        ...
        {
            var prop = typeof (SqlParameter).GetProperty("TypeName",
    ...
    partial class FeatureSupport
    {
        ...
        {
            var name = connection == null ? null : connection.GetType().Name;
    ...
    sealed partial class DefaultTypeMap : SqlMapper.ITypeMap
    {
        ...
        public ConstructorInfo FindConstructor(string[] names, Type[] types)
        {
            ...
            foreach (
                var ctor in
                    constructors.OrderBy(c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1))
...
#else
            var withAttr =
                constructors.Where(c => c.GetCustomAttributes(typeof (ExplicitConstructorAttribute), true).Length > 0)
                    .ToList();
#endif
        ...
        public SqlMapper.IMemberMap GetMember(string columnName)
        {
            var property = _properties.FirstOrDefault(
                p => string.Equals(p.Name, columnName, StringComparison.Ordinal))
                           ??
            ...
            {
                property = _properties.FirstOrDefault(
                    p => string.Equals(p.Name, columnName.Replace("_", ""), StringComparison.Ordinal))
                           ??
                           _properties.FirstOrDefault(
                               p =>
            ...
            {
                field = _fields.FirstOrDefault(
                    p => string.Equals(p.Name, columnName.Replace("_", ""), StringComparison.Ordinal))
                        ??
                        _fields.FirstOrDefault(
                            p => string.Equals(p.Name, columnName.Replace("_", ""), StringComparison.OrdinalIgnoreCase));
            }
